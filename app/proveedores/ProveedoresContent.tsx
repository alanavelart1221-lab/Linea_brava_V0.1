...
export function ProveedoresContent({ providers }: { providers: Provider[] }) {
  return (
    <Suspense>
      <ProveedoresInner providers={providers} />
    </Suspense>
  );
}

function ProveedoresInner({ providers }: { providers: Provider[] }) {
  const searchParams = useSearchParams();
  const [typeFilter, setTypeFilter] = useState<Filter>(ALL);
  const [stateFilter, setStateFilter] = useState<string>(ALL);

  const states = useMemo(
    () => [...new Set(providers.map((p) => p.state))].sort(),
    [providers]
  );
  ...
  }, [providers, typeFilter, stateFilter]);
          ...
          <div className="mb-10 flex flex-col gap-4 rounded-2xl border border-trail-500/30 bg-trail-500/5 p-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-mute">
                Solicita aparecer en el directorio. Revisamos tu solicitud y te publicamos.
              </p>
            </div>
            <Link
          ...
          {filtered.length > 0 ? (
            ...
            <div className="flex flex-col items-center gap-3 py-20 text-center">
              ...
              <p className="text-sm text-mute">
                {providers.length === 0
                  ? "Aún no hay proveedores publicados. Sé el primero en solicitar tu registro."
                  : "Prueba otro filtro o estado."}
              </p>
              {providers.length > 0 && (
                <button
